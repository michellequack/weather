...
! function() {
    ...
    window.W = { version: "19.10.3", assets: "19.10.3.emb.985a", target: "embed2", build: "2019-09-02, 15:34", startTs: Date.now() };
}(),
/*!
...
L.CanvasLayer = L.Layer.extend({
    ...
    W.define("ga", ["rootScope", "store", "utils"], function(i, e, t) {
        ...
        return 5 !== Math.round(10 * Math.random()) && (o = t.emptyFun), {
            pageview: o
        }
    }), /*! */
    ...
    W.define("store", ["dataSpecifications", "utils", "storage", "rootScope", "Evented", "broadcast"], function(r, n, a, o, e, t) {
        ...
        return f("firstUserSession") || s("firstUserSession", Date.now()), o.sessionCounter = f("sessionCounter201803") + 1, s("sessionCounter201803", o.sessionCounter), t.emit("identityCreated", g), t.emit("provisionaryToken", a.get("userToken")), t.on("tokenRecieved", function(e) {
        ...
        return {
            get: function(e, t) {
                ...
                return new Promise(function(f) {
                    i.get("/reverse/v3/" + m + "/" + p + "/" + n + "?lang=" + g).then(function(e) {
                        var t = e.data,
                            n = t.locality,
                            i = t.suburb,
                            s = t.city,
            ...
            tilesReady: function(N, e, W) {
                var I = this,
                    r = function(e, t) {
                        ...
                        if (W.PNGtransparency && k.testPNGtransparency(h, f)) return NaN;
                        if (W.JPGtransparency && k.testJPGtransparency(h, f)) return NaN;
                        var m = h[f],
                            p = h[f + 4],
                            g = h[f + 1],
                            v = h[f + 5],
                            ...
                            x = d * d,
                            D = (m * M + p * _ + b * C + T * P) / x,
                            R = W.interpolateNearestG ? k.interpolateNearest(null, 0, g, v, L, S, M, _, C, P) : (g * M + v * _ + L * C + S * P) / x,
                            O = (w * M + y * _ + A * C + E * P) / x;
    ...
    W.define("renderTile", ["render", "layers"], function(te, ne) {
        return function(e, t, n, i, s) {
            if (n === this.syncCounter) {
                ...
                var u, h, f, m, p, g, v, w, y, b, T, L, S = !1,
                    A = 0 | i.trans,
                    E = 0 | Math.log2(A),
                    M = 0 | Math.log2(A * A),
                    _ = 0 | i.intX,
                    ...
                    K = s.decodeR,
                    ee = s.decodeG;
                for (r ? (y = te.createCombinedFillFun(c, Q.getColor(), Q.getColor2(), Q.getAmountByColor), b = te.createFillFun(c, e, Q.getColor())) : y = b = te.createFillFun(c, e, Q.getColor()), $ && (K = s.decodeB), L = 0; L < 256; L += e)
                    for (z = L - ((k = L >> E) << E), T = 0; T < 256; T += e) G = T - ((I = T >> E) << E), U !== I && (R = 2056 + I + O + (((F = k + N) << 8) + F) << 2, void 0 !== d && (S = d(o, R)), !0 == $ && (R += 2), H = o[R], B = o[R + 4], q = o[R + 1028], j = o[R + 1032], !0 == J && (V = o[R + 1], Y = o[R + 5], Z = o[R + 1029], X = o[R + 1033]), U = I), S ? b(T, L, NaN) : (v = K(null !== x ? H * x[D = G + (z << E) << 2] + B * x[D + 1] + q * x[D + 2] + j * x[D + 3] >> M : H * (u = (1 - (p = G / A)) * (1 - (g = z / A))) + B * (h = p * (1 - g)) + q * (f = g * (1 - p)) + j * (m = p * g)), !0 == J && (w = ee(null !== x ? V * x[D] + Y * x[D + 1] + Z * x[D + 2] + X * x[D + 3] >> M : V * u + Y * h + Z * f + X * m)), r ? y(T, L, v, w) : y(T, L, J ? Math.sqrt(v * v + w * w) : v));
                l.putImageData(te.imgData, 0, 0), "pattern" in a && a.pattern in W && W[a.pattern].addPattern(l, A, o, 2056, O, N, P, K, ee)
            }
        }
    }), /*! */
        ...
        return L.GridLayer.extend({
            ...
            renderTile: function(e, t, n) {
                for (var i = this._getTilePos(t), s = i.x, a = i.y, r = this._map.getPixelOrigin(), o = r.x, l = r.y, c = 256 << t.z, d = [], u = 0; u < n.length; ++u) {
                    ...
                    A.textContent = A.dataset.label = m, A.dataset.id = T, A.dataset.poi = "label", A.className = p, A.style.transform = "translate(" + L + "px, " + S + "px)", A.style.width = w + "px", b || d.push({
                        id: T,
                        el: A
                    }), e.appendChild(A)
                }
    ...
    W.define("storage", ["rootScope", "utils"], function(a, e) {
        var n = {};
        return {
            isAvbl: !1,
            getFile: function(i) {
                ...
                return new Promise(function(t, n) {
                    e.loadScript(a.assets + "/" + i.replace(".json", ".js")).then(function() {
                        var e = W.requestedFile;
                        e && "object" == typeof e ? (e.version = a.version, s.put(i, {
                            version: a.version,
                            data: e
                        }), t(e)) : n()
                    }).catch(function(e) {
                        window.wError("storage", "Failed to load lang file as .js", e), n()
                    })
                })
            },
            put: function(e, t) {
...
var q = /^\?\S+,\S+,\S+/.test(window.location.search) ? parseOldParams() : parseNewParams(),
    wrq = W.require,
    parseUrl = function() {
        var e = wrq("store"),
            t = wrq("utils");
        if ("coordinates" === q.location && q.lat && q.lon && (e.set("startUp", "location"), e.set("homeLocation", {
                lat: q.lat,
                lon: q.lon,
                zoom: Math.min(11, +q.zoom),
                source: "fav"
            })), q.level && e.set("level", q.level), q.overlay && e.set("overlay", q.overlay), q.product && e.set("product", q.product), q.pressure && e.set("isolines", "pressure"), "map" === q.type && /in|12|24/.test(q.calendar)) {
            var n = /12|24/.test(q.calendar) ? q.calendar : q.forecast;
            e.set("timestamp", Date.now() + +n * t.tsHour)
        }
    },
    injectLanguages = function() {
        var e = wrq("rootScope"),
            t = wrq("plugins").detail;
        q.lang && (e.prefLang = q.lang), "alert" === q.type && (e.isMobile = !0), t.replaceMountingPoint = !1, t.dependencies = ["nearest", "detail-render"], t.attachPoint = "#plugins"
    };
parseUrl(), injectLanguages(), wrq("trans");
var ref = document.referrer,
    base = "https://www.windy.com/",
    ...
    alertShot = function() {
        var e = wrq("broadcast"),
            t = wrq("overlays"),
            n = wrq("favs");
        (0, wrq("ga").pageview)("/embedAlertShot"), document.body.classList.add("alert-shot"), W.loadOrphanedModules(), t.wind.setMetric(q.metricWind || "kt"), t.temp.setMetric(q.metricTemp || "°C"), n.hasTimestamps = function() {
            return q.timestamps && q.timestamps.split("-").map(function(e) {
                return +e
            })
        }, e.emit("rqstOpen", "detail", q)
    },
    embedFcst = function() {
        var e = wrq("broadcast"),
            t = wrq("$"),
            n = wrq("overlays");
        (0, wrq("ga").pageview)("/embedForecast"), document.body.classList.add("embed-forecast"), W.loadOrphanedModules(), q.metricWind && n.wind.setMetric(q.metricWind || "kt"), q.metricTemp && n.temp.setMetric(q.metricTemp || "°C");
        var i = {
            lat: +q.detailLat,
            lon: +q.detailLon,
            zoom: Math.min(11, +q.zoom),
        ...
        e.emit("rqstOpen", "detail", i), e.once("detailRendered", function() {
            var e = document.createElement("a");
            e.href = base + q.lat + "/" + q.lon + "?" + utm, e.className = "embed-link", e.target = "_top", t("#detail-box").appendChild(e)
        })
    },
    embedMap = function() {
        var t = wrq("rootScope"),
            ...
            r = wrq("map"),
            a = wrq("ga").pageview;
        document.body.classList.add("embed-map"), W.loadOrphanedModules(), q.metricWind && "default" !== q.metricWind && s.set("metric_wind", q.metricWind, {
            forceChange: !0
        }), q.metricTemp && "default" !== q.metricTemp && s.set("metric_temp", q.metricTemp, {
            forceChange: !0
        }), "station" === q.type ? a("/embedStation/" + q.id) : a("/embedMap" + (q.detail ? "-withDetail" : "") + (q.marker ? "-withMarker" : "") + (q.oldVersion ? "-oldURL" : "") + "/" + (q.overlay || "wind")), s.setDefault("favOverlays", t.overlays), s.setDefault("wasDragged", !0), q.radarRange && s.set("radarRange", q.radarRange), q.menu && document.body.classList.add("hide-controls"), q.message && document.body.classList.add("hide-message");
        var o = function(e, t) {
            var n = e.lat,
                i = e.lon,
                s = r.latLngToContainerPoint([n, i]).y,
        ...
        if (q.detail) {
            ...
            e.emit("rqstOpen", "detail", l), e.once("detailRendered", function() {
                return o(l, 180)
            })
        } else "station" === q.type && q.id ? (e.emit("rqstOpen", "station", q.id), e.once("stationLoaded", function(e) {
            o(e, 220);
            var t = n('#station-detail-box [data-do="fcst"]');
            t && (t.outerHTML = '<a target="_top"\n\t\t\tdata-do="url,https://www.windy.com/station/' + q.id + "?" + utm + '">\n\t\t\tDetailed data for this weather station</a>')
        })) : q.marker && e.once("redrawFinished", function() {
            e.emit("rqstOpen", "picker", q)
        });
        r.scrollWheelZoom.disable(), window.addEventListener("focus", function() {
            r.scrollWheelZoom.enable(), a("/embedMap/interaction")
        }), e.once("redrawFinished", function() {
            setTimeout(function() {
                return e.emit("uiChanged")
            }, 500)
        }), e.on("redrawFinished", function(e) {
            search = [e.overlay, e.level, t.map.lat.toFixed(3), t.map.lon.toFixed(3), t.map.zoom].join(","), n("#note-message").href = n("#logo").href = base + "?" + search + utm
        }), q.embedMake && window.parent && window.parent.updateValues && (i.on("rendered", function(e) {
            window.parent.updateDetail(e)
        }), e.on("redrawFinished", function(e) {
            window.parent.updateValues(t.map, e)
        }), s.on("radarRange", function(e) {
            window.parent.updateRange(e)
        }))
    };

function parseNewParams() {
    for (var e = {}, t = window.location.hash.substring(1), n = window.location.search.substring(1), i = (t || n).split("&"), s = 0; s < i.length; s++) {
        var a = i[s].split("=");
...
function parseOldParams() {
    for (var e = window.location.search.substring(1).split(","), t = {}, n = {
            levels: ["surface", "100m", "975h", "950h", "925h", "900h", "850h", "800h", "700h", "600h", "500h", "400h", "300h", "250h", "200h", "150h"],
            overlays: ["wind", "gust", "rain", "temp", "dewpoint", "clouds", "lclouds", "mclouds", "hclouds", "cbase", "visibility", "rainAccu", "snowAccu", "rh", "cape", "pressure", "waves", "swell", "swell1", "swell2", "swell3", "wwaves", "sst", "currents"]
        }, i = 0; i < e.length; i++) {
        if (/^-?\d+\.\d+$/.test(e[i]) && /^-?\d+\.\d+$/.test(e[i + 1]) && /^\d+$/.test(e[i + 2])) {
            var s = parseInt(e[i + 2]);
            t.lat = parseFloat(e[i]), t.lon = parseFloat(e[i + 1]), t.zoom = 100 < s ? 4 : s, t.location = "coordinates", i += 2
        }
        var a = e[i];
        if (-1 < n.levels.indexOf(a) && (t.level = a), -1 < n.overlays.indexOf(a) && (t.overlay = a), "message" === a && (t.message = !0), "ip" === a && (t.location = "ip"), "marker" === a && (t.marker = !0), /metric/.test(a)) {
            var r = a.match(/metric\.(.+)\.(.+)/);
            1 === r[2].length && (r[2] = "°" + r[2]), t[r[1]] = r[2]
        }
    }
    return t.oldVersion = !0, console.log("%c Aloha! We have faster and more detailed version of Windy widget for you. Please update your code at https://www.windy.com/widgets", "color: blue; font-size: 200%;"), t
}
document.addEventListener("DOMContentLoaded", function() {
    switch (document.documentElement.id = "device-not-set", q.type) {
        case "alert":
            alertShot();
            break;
        case "forecast":
            ...
            break;
        case "station":
            document.documentElement.id = "device-desktop", embedMap();
            break;
        default:
            embedMap()
    }
